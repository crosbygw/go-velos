...
*/
// Package velos interacts with F5 velos chassis/systems using the REST API.
package velos
...
// VELOS is a container for our session state.
type VELOS struct {
	Host      string
	User      string
	Password  string
...
func NewSession(host, port, user, passwd string, configOptions *ConfigOptions) *VELOS {
	var url string
...
func NewTokenSession(host, port, user, passwd, loginProviderName string, configOptions *ConfigOptions) (b *VELOS, err error) {
	type authReq struct {
...
func (b *VELOS) APICall(options *APIRequest) ([]byte, error) {
...
func (b *VELOS) iControlPath(parts []string) string {
...
//Generic delete
func (b *VELOS) delete(path ...string) error {
	req := &APIRequest{
		Method: "delete",
...
//Generic delete
func (b *VELOS) deleteReq(path ...string) ([]byte, error) {
	req := &APIRequest{
		Method: "delete",
		URL:    b.iControlPath(path),
	}
...
func (b *VELOS) deleteReqBody(body interface{}, path ...string) ([]byte, error) {
...
func (b *VELOS) post(body interface{}, path ...string) error {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return err
	}
...
func (b *VELOS) postReq(body interface{}, path ...string) ([]byte, error) {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return nil, err
	}
...
func (b *VELOS) put(body interface{}, path ...string) error {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return err
	}
...
func (b *VELOS) putReq(body interface{}, path ...string) ([]byte, error) {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return nil, err
	}
...
func (b *VELOS) patch(body interface{}, path ...string) error {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return err
	}
...
func (b *VELOS) fastPatch(body interface{}, path ...string) ([]byte, error) {
	marshalJSON, err := jsonMarshal(body)
	if err != nil {
		return nil, err
	}
...
func (b *VELOS) Upload(r io.Reader, size int64, path ...string) (*Upload, error) {
	client := &http.Client{
		Transport: b.Transport,
		Timeout:   b.ConfigOptions.APICallTimeout,
	}
...
func (b *VELOS) getForEntity(e interface{}, path ...string) (error, bool) {
	req := &APIRequest{
		Method:      "get",
		URL:         b.iControlPath(path),
		ContentType: "application/json",
...
func (b *VELOS) getForEntityNew(e interface{}, path ...string) (error, bool) {
	req := &APIRequest{
		Method:      "get",
		URL:         b.iControlPath(path),
		ContentType: "application/json",
...
func (b *VELOS) checkError(resp []byte) error {
	if len(resp) == 0 {
		return nil
	}
